...
As shown in Figure \ref{fig:thread_scalability}, the execution time initially follows a standard scaling trend, with significant performance gains moving from 1 to 4 threads. However, the scalability curve exhibits two noteworthy phenomena: a performance regression at **5 threads** and a subsequent plateau starting from **6 threads**.

The "hump" observed at 5 threads (where execution time rises to $\sim 0.0051$s from the $\sim 0.0048$s of the 4-thread configuration) represents a classic case of parallel overhead or hardware contention. In the context of the **Intel Xeon Sapphire Rapids** architecture, this behavior can be attributed to:

\begin{itemize}
    \item \textbf{Non-Optimal Workload Distribution}: Since the matrix dimension ($500 \times 500$) and the Cannon's algorithm sub-blocks are likely power-of-two or multiples of 4, distributing this workload across 5 threads creates an uneven decomposition. This leads to load imbalance where some threads idle while others finish their computation.
    \item \textbf{Cache and Mesh Interconnect Contention}: Sapphire Rapids uses a mesh topology to connect cores and cache slices. Moving from 4 to 5 threads may involve crossing a tile boundary or a different mesh row, increasing the latency for cache coherency traffic (L3 RFO - Request For Ownership) and snoop filter lookups.
    \item \textbf{Resource Oversubscription}: If the MPI task is pinned to a specific NUMA node or a set of cores sharing execution resources (like ALUs or AVX-512 units), the 5th thread might be competing for the same physical resources, negating the benefits of additional parallelism.
\end{itemize}

Beyond 6 threads, the system reaches a scaling limit where the execution time stabilizes around $0.0035$s. At this stage, the communication overhead of the MPI exchange phases and the overhead of managing the OpenMP thread pool outweigh the computational speedup, confirming the practical limits defined by Amdahl's Law for this specific matrix size.
