...
The application requires the execution of a 2D stencil operation to process multi-dimensional data. The baseline operation consists of applying a specific $3\times3$ weighted average filter, denoted as $W$, to mitigate image noise \cite{assignment}. 
...
Following the \texttt{\_\_syncthreads()} barrier to ensure memory consistency, the convolution math is executed entirely out of the L1/Shared Memory space, reducing global memory accesses by nearly a factor of 9 and drastically increasing the effective memory bandwidth.
...
Execution times were gathered across multiple runs for 4K, 8K, and 16K images using varying block sizes (Figure \ref{fig:block_size_impact}). The empirical data highlights the deterministic nature of the GPU hardware. For instance, processing a 16K image strictly required an average of $76.48$~ms (with a $32\times32$ block size) on a single A100 GPU, regardless of the noise density applied to the input (ranging from $0\%$ to $90\%$), as depicted in Figure \ref{fig:simd_determinism}. 
...
This analysis strictly validates that while linear low-pass filters are effective against Gaussian (thermal) noise, non-linear rank-order filters (such as the Median filter) are mandatory to successfully reject non-additive impulse noise without degrading the high-frequency components (edges) of the underlying signal.
...
\begin{figure}[h!]
    \includesvg[width=0.85\linewidth]{}
    \caption{Execution time stability across varying impulse noise densities, demonstrating hardware determinism.}
    \label{fig:simd_determinism}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption{Comparative hardware execution time scaling (RTX 2060 vs. Multi-GPU A100).}
    \label{fig:hw_comparison_32}
\end{figure}

% Grafico 4: PSNR Degradation
\begin{figure}[htbp]
    \centering
    \includesvg[width=0.85\textwidth]{}
    \caption{Catastrophic PSNR degradation of the linear $3\times3$ filter when subjected to high-density impulse noise.}
    \label{fig:psnr_degradation}
\end{figure}

% Qui puoi includere la Tabella generata in automatico dallo script Python
% \input{statistics_table.tex}
