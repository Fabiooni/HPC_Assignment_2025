...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\linewidth]{thread_scalability_500.png}
...
\begin{itemize}
    \item \textbf{Cache Locality and Tiling}: Sapphire Rapids utilizes a multi-die (tile-based) architecture. With 3 threads, the sub-blocks of the $500 \times 500$ matrix might reach an optimal size that fits entirely within the L2 or L3 cache slices associated with the active cores, drastically reducing memory latency.
    \item \textbf{Memory Controllers and Bandwidth}: The transition to 3 threads may trigger a more efficient use of the integrated Memory Controllers (iMCs), especially if the workload distribution better aligns with the mesh interconnect topology of the Sapphire Rapids socket.
    \item \textbf{AVX-512 and AMX Utilization}: If the implementation utilizes Intel Advanced Matrix Extensions (AMX), the 3-thread configuration might reach the critical threshold for hardware acceleration that was underutilized with only 2 threads.
\end{itemize}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\linewidth]{matrix_dimension_comparison.png}
