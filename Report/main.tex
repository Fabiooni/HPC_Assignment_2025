...
This exercise demonstrates the implementation of parallel matrix multiplication on a CPU-based cluster (with more the one node). The solution adopts a hybrid computing model, combining MPI (Message Passing Interface) for distributed memory management across multiple processes and OpenMP for multi-threaded local computation within ciascun process. Specifically, the code implements Systolic Array Algorithm, which is optimized for mesh-connected parallel processors.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\linewidth]{MPI/thread_scalability_500.jpeg}
    \caption{Average execution time as a function of threads per MPI task ($500 \times 500$ matrix).}
    \label{fig:thread_scalability}
\end{figure}

As shown in Fig. \ref{fig:thread_scalability}, performance follows Amdahl’s Law but exhibits an anomaly at **5 threads**, where execution time rises to $\sim 0.0051$s before dropping to a minimum at 6 threads.

\textbf{Memory Footprint Analysis:}
With a $500 \times 500$ matrix split over two nodes ($250 \times 250$ sub-matrices):
\begin{itemize}
    \item $250 \times 250 \times 4 \text{ bytes (float)} \approx 244 \text{ KB}$ per matrix.
    \item Total footprint (5 matrices): $\sim 1.19 \text{ MB} < \text{L2 cache (2 MB)}$.
\end{itemize}
This confirms the data is L2-resident; thus, the regression is likely architectural.

\textbf{Hypotheses for the 5-thread regression:}
\begin{itemize}
    \item \textbf{Hardware Conflict}: Unbalanced thread mapping across Sub-NUMA Clusters (SNC) or tiles, causing resource contention.
    \item \textbf{Mesh Latency}: Increased traffic across the internal Mesh Interconnect (UPI) when crossing tile boundaries with an odd thread count.
    \item \textbf{Synchronization Overhead}: OpenMP management costs for 5 threads may outweigh the computational gains for a relatively small $500 \times 500$ workload.
\end{itemize}
...
\end{itemize}
The theoretical scaling factor for doubling $N$ is $2^3 = 8$. The slight overhead observed at $N=1000$ is likely due to the increased communication volume in Systolic Array algorithm, as the MPI message sizes for sub-block swaps grow quadratically ($N^2$) with the matrix dimension.

\textbf{Memory Occupation per Matrix:}
\begin{itemize}
    \item \textbf{500 $\times$ 500}: $250,000 \times 4 \text{ B} = 1,000,000 \text{ B} \approx \mathbf{0.95 \text{ MiB}}$ (Fits in L2 cache)
    \item \textbf{1000 $\times$ 1000}: $1,000,000 \times 4 \text{ B} = 4,000,000 \text{ B} \approx \mathbf{3.81 \text{ MiB}}$ (Exceeds L2 per-core, fits in L3)
    \item \textbf{2000 $\times$ 2000}: $4,000,000 \times 4 \text{ B} = 16,000,000 \text{ B} \approx \mathbf{15.26 \text{ MiB}}$ (Heavy L3/DRAM usage)
\end{itemize}
...
\input{CUDA/statistics_table.tex}
